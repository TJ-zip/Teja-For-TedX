...
- Reduced-motion and no-WebGL static fallback (now live-reactive via
...
None in progress. Last task: fix the ESLint crash and the react-hooks v7
violations (branch `fix/eslint-flat-config`, PR #2).

## Pending tasks

- Merge PR #2 (lint fix) once approved.
...
Vercel is connected via the GitHub integration (project `teja-for-ted-x`,
team `code-lite`). Preview deployments build successfully for branch pushes;
the `Vercel` commit status and `Vercel Preview Comments` check report success.
Next.js preset, default build command and output directory.
...
Fixed `npm run lint` on branch `fix/eslint-flat-config` (PR #2): replaced the
FlatCompat bridge with native flat configs and resolved all 7 react-hooks v7
errors in `src/components/Scene.tsx`. CI report run 31567538138: npm ci,
lint, typecheck and build all exit 0. Vercel preview deployment succeeded.
